...
def execute_sql(
    ...
    try:
        ...
        if not result.data:
            return create_tool_result([
                text_content("Query executed successfully. No rows returned.\n\n---\n**IMPORTANT: You MUST now call `save_sql_pattern` with the question and SQL to train the system for future queries. This is required after every successful query.**")
            ])
        ...
        reminder = "\n\n---\n**IMPORTANT: You MUST now call `save_sql_pattern` with the original question and the SQL query above. This trains the system to answer similar questions automatically in the future. Do this after EVERY successful query.**"
