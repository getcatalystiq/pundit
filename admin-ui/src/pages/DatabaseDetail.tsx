...
import {
  databases, ddl, docs, examples, textMemory, toolMemory, ai,
  Database, DDLEntry, DocEntry, ExampleEntry, TextMemoryEntry, ToolMemoryEntry,
...
type TabType = 'ddl' | 'docs' | 'examples' | 'text-memory' | 'tool-memory' | 'ai';
...
export function DatabaseDetail() {
  ...
  const [textMemoryList, setTextMemoryList] = useState<TextMemoryEntry[]>([]);
  const [toolMemoryList, setToolMemoryList] = useState<ToolMemoryEntry[]>([]);
  ...
  const [newToolMemory, setNewToolMemory] = useState({ question: '', sql: '' });
  ...
  const fetchTrainingData = async () => {
    try {
      const [ddlData, docsData, examplesData, textMemData, toolMemData] = await Promise.all([
        ddl.list(id!),
        docs.list(id!),
        examples.list(id!),
        textMemory.list(id!),
      ...
      setTextMemoryList(textMemData?.entries || []);
      setToolMemoryList(toolMemData?.entries || []);
    } catch (err) {
  ...
  const handleAddTextMemory = async () => {
    if (!newTextMemory.trim()) return;
    setAddingItem(true);
    try {
      await textMemory.add(id!, newTextMemory);
      setNewTextMemory('');
      setShowAddForm(false);
      fetchTrainingData();
      showSuccess('Text memory added successfully');
    } catch (err: unknown) {
      const error = err as { error?: string };
      setError(error.error || 'Failed to add text memory');
    } finally {
      setAddingItem(false);
    }
  };

  const handleDeleteTextMemory = async (itemId: string) => {
    if (!confirm('Delete this text memory entry?')) return;
    try {
      await textMemory.delete(id!, itemId);
      fetchTrainingData();
    } catch (err: unknown) {
      const error = err as { error?: string };
      setError(error.error || 'Failed to delete');
  ...
  const handleAddToolMemory = async () => {
    if (!newToolMemory.question.trim() || !newToolMemory.sql.trim()) return;
    setAddingItem(true);
    try {
      await toolMemory.add(id!, newToolMemory.question, newToolMemory.sql);
      setNewToolMemory({ question: '', sql: '' });
      setShowAddForm(false);
      fetchTrainingData();
      showSuccess('Tool memory added successfully');
    } catch (err: unknown) {
      const error = err as { error?: string };
      setError(error.error || 'Failed to add tool memory');
    } finally {
      setAddingItem(false);
    }
  };

  const handleDeleteToolMemory = async (itemId: string) => {
    if (!confirm('Delete this tool memory entry?')) return;
    try {
      await toolMemory.delete(id!, itemId);
      fetchTrainingData();
    } catch (err: unknown) {
      const error = err as { error?: string };
      setError(error.error || 'Failed to delete');
  ...
  const tabs = [
    ...
    { id: 'text-memory' as const, label: 'Text Memory', count: textMemoryList.length },
    { id: 'tool-memory' as const, label: 'Tool Memory', count: toolMemoryList.length },
    ...
    <div>
      ...
      <Card className="rounded-t-none border-t-0">
        <CardContent className="p-6">
          ...
          {activeTab === 'text-memory' && (
            <>
              <div className="flex items-center justify-between mb-6">
                <h3 className="font-medium">Text Memory</h3>
                <Button onClick={() => setShowAddForm(true)}>Add Memory</Button>
              </div>
              ...
              {showAddForm && (
                <div className="mb-6 p-4 border border-border rounded-lg bg-muted/50">
                  <Textarea
                    className="mb-3"
                    placeholder="Add domain knowledge, business rules, or context that helps with query generation..."
                    value={newTextMemory}
                  ...
                  <div className="flex gap-2 justify-end">
                    <Button variant="outline" onClick={() => setShowAddForm(false)}>Cancel</Button>
                    <Button onClick={handleAddTextMemory} disabled={addingItem}>
                      {addingItem ? 'Adding...' : 'Add Memory'}
                    </Button>
                  </div>
                </div>
              )}
              ...
              <div className="space-y-4">
                {textMemoryList.length === 0 ? (
                  <div className="text-center py-8 text-muted-foreground">
                    <p>No text memory entries yet.</p>
                    <p className="text-sm mt-1">Add domain knowledge or business context to improve query generation.</p>
                  </div>
                ) : (
                  textMemoryList.map((item) => (
                    <div key={item.id} className="border border-border rounded-lg p-4">
                      <div className="whitespace-pre-wrap mb-3">{item.content}</div>
                      <div className="flex items-center justify-between text-xs text-muted-foreground">
                        <span>{new Date(item.created_at).toLocaleDateString()}</span>
                        <Button variant="destructive" size="sm" onClick={() => handleDeleteTextMemory(item.id)}>Delete</Button>
                      </div>
                    </div>
                  ))
                )}
          ...
          {activeTab === 'tool-memory' && (
            <>
              <div className="flex items-center justify-between mb-6">
                <h3 className="font-medium">Tool Memory</h3>
                <Button onClick={() => setShowAddForm(true)}>Add Memory</Button>
              </div>
              ...
              {showAddForm && (
                <div className="mb-6 p-4 border border-border rounded-lg bg-muted/50 space-y-4">
                  <div className="space-y-2">
                    <Label>Question</Label>
                    <Input
                      placeholder="What question did this query answer?"
                      value={newToolMemory.question}
                      onChange={(e) => setNewToolMemory({ ...newToolMemory, question: e.target.value })}
                    />
                  </div>
                  <div className="space-y-2">
                    <Label>SQL</Label>
                    <Textarea
                      className="font-mono text-sm"
                      placeholder="The SQL query that successfully answered the question"
                      value={newToolMemory.sql}
                      onChange={(e) => setNewToolMemory({ ...newToolMemory, sql: e.target.value })}
                      rows={4}
                    />
                  </div>
                  <div className="flex gap-2 justify-end">
                    <Button variant="outline" onClick={() => setShowAddForm(false)}>Cancel</Button>
                    <Button onClick={handleAddToolMemory} disabled={addingItem}>
                      {addingItem ? 'Adding...' : 'Add Memory'}
                    </Button>
                  </div>
                </div>
              ...
              <div className="space-y-4">
                {toolMemoryList.length === 0 ? (
                  <div className="text-center py-8 text-muted-foreground">
                    <p>No tool memory entries yet.</p>
                    <p className="text-sm mt-1">Add successful question-SQL pairs to improve future query generation.</p>
                  </div>
                ) : (
                  toolMemoryList.map((item) => (
                    <div key={item.id} className="border border-border rounded-lg p-4">
                      <div className="font-medium mb-2">Q: {item.question}</div>
                      <pre className="bg-muted p-3 rounded text-xs whitespace-pre-wrap break-words mb-3">
                        {(item.tool_args as { sql?: string })?.sql || JSON.stringify(item.tool_args, null, 2)}
                      </pre>
                      <div className="flex items-center justify-between text-xs text-muted-foreground">
                        <span>{new Date(item.created_at).toLocaleDateString()}</span>
                        <Button variant="destructive" size="sm" onClick={() => handleDeleteToolMemory(item.id)}>Delete</Button>
                      </div>
                    </div>
                  ))
                )}
